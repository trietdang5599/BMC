...
from baselines.DDQL.data_processor import DDQLDataProcessorForRecommendation, DDQLTorchDataset, \
    DDQLDataProcessorForNegotiation, DDQLDataProcessorForEmotionalSupport
        ...
        for i in tqdm(range(self.model_config.num_train_ppo_epochs)):
            ...
            for batch in train_loader:
                ...
                if self.model_config.prioritized_objective == "uniform":
                    sampled_preferences = random_weights(self.model_config.n_objectives, n=1)
                else:
                    sampled_preferences = np.array(self.model_config.obj_to_weight[self.model_config.prioritized_objective.strip()])


                sampled_preferences = torch.Tensor(sampled_preferences).to(self.device).requires_grad_(False)
                sampled_preferences = sampled_preferences.unsqueeze(0)
