...
from baselines.PADPP_min_dist.data_processor import MinDistPADPPDataProcessorForRecommendation, MinDistPADPPTorchDataset, \
    MinDistPADPPDataProcessorForNegotiation, MinDistPADPPDataProcessorForEmotionalSupport
...
class MinDistPADPPTrainer(Trainer):
    ...
    def construct_dataloaders(self, data_instances, batch_size, goal2id, shuffle=True, num_workers=1):
        ...
        if self.game_config.name == RECOMMENDATION:
            convert_example_to_feature = MinDistPADPPDataProcessorForRecommendation
        # the data processor for the negotiation scenario
        elif self.game_config.name == NEGOTIATION:
            convert_example_to_feature = MinDistPADPPDataProcessorForNegotiation
        # the data processor for the emotional support conversation
        elif self.game_config.name == EMOTIONAL_SUPPORT:
            convert_example_to_feature = MinDistPADPPDataProcessorForEmotionalSupport
        else:
            raise Exception("Invalid scenario....")
        # construct the torch dataset
        torch_dataset = MinDistPADPPTorchDataset(
        ...
        for i in tqdm(range(self.model_config.num_train_ppo_epochs)):
            ...
            for batch in train_loader:
                ...
                with torch.no_grad():
                    ...
                    Q_prime = Q_prime.view(-1, self.model_config.n_preferences, action_size, self.model_config.n_objectives)

                    # bs, n_sample * n_action, 2
                    # 1, 2, n_sample
                    repeated_sample_preference = sampled_preferences.repeat(1, bs).view(-1, self.model_config.n_objectives)
                    repeated_prev_sampled_preferences = prev_sampled_preferences.repeat(bs, 1).view(-1, self.model_config.n_objectives)
                    
                    # computing the cosine similarity between current updated and past updated preferences
                    co_sim = torch.nn.functional.cosine_similarity(repeated_sample_preference, repeated_prev_sampled_preferences).view(-1, self.model_config.n_preferences)
                    
                    # taking the min distance i.e minimal cosine similarity
                    min_dist_ids = co_sim.max(1)[1]
                    min_dist_ids = min_dist_ids.repeat(bs, 1).view(-1, self.model_config.n_preferences)

                    # select the Q values accoridng to the min distance ids
                    Q2 = Q_prime.gather(1, min_dist_ids.view(-1, self.model_config.n_preferences, 1, 1).expand(min_dist_ids.size(0),
                                                    min_dist_ids.size(1), action_size,
                                                    self.model_config.n_objectives)).squeeze() 
                    
                    # reshape the target Q function
                    Q2 = Q2.view(-1, action_size, self.model_config.n_objectives)
                                        
                    # taking the argmax over scalarizing action
                    repeated_sampled_preferences = sampled_preferences.repeat(bs, 1).view(-1, self.model_config.n_objectives)
                    
                    # scalarized scores acording to the sampled prefrerences 
                    scalarized_scores = torch.bmm(repeated_sampled_preferences.unsqueeze(1), Q2.permute(0,2,1)).squeeze(1)
                    scalarized_ids = scalarized_scores.max(1)[1]
                    ...
                    # print(idx)
                    # collect target Q
                    Q2 = Q2.gather(1, scalarized_ids.view(-1, 1, 1).expand(scalarized_ids.size(0),
                                                                     1,
                                                                     self.model_config.n_objectives)).squeeze()                 
