...
from config.constants import RECOMMENDATION, NEGOTIATION, EMOTIONAL_SUPPORT, SL_RATIO, SUCCESS_RATE, AVG_TURN, FAIRNESS, \
    TOXICITY, USER_REWARD, PERSUATION


class ICLAIFTrainer(Trainer):
    ...
    def predict(self, instance, action_mapping=None, domain = None):
        """
        ...
        if self.game_config.name == RECOMMENDATION:
            action = self.model(instance['dialogue_context'],
                                target_item = instance['task_background']['target_topic'], # for the recommendation scenario
                                domain = domain, # for the recommendation scenario
    ...
    def online_test(self, cases, device=None, simulators=None, action_mapping=None, domain = None):
        """
        ...
        for idx, (case, simulator) in tqdm(enumerate(list(zip(cases, simulators)))):
            ...
            # dialogue_level strategy
            dialogue_level_action = None
            ...
            for t in count():  # user  dialog
                ...
                # predict the action
                # for ICL_AIF model
                # only predict the action once
                if dialogue_level_action is None:
                    action = self.predict(state, action_mapping, domain = domain)
                    dialogue_level_action = action
                # using the predicted dialogue action
                else:
                    action = dialogue_level_action
