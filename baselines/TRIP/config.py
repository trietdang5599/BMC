...
class TRIPConfigForRecommendation(TRIPConfig):
    combined_action = False
    special_tokens_dict = rec_special_tokens_dict
    objective_weight = [1.0, 1.0]
    temperature = 0.000001
    prompt = """
    You are an expert in item recommendation.
    Now give you a conversation history between a system and a user, you need to infer the mental states
    and future actions of the user.
    """
    cot_prompt = """
    The following is the conversation history: {}
    Question: What are the mental states and future actions of the user.? Answer:
    """
    """
    Class TRIP configuration for recommendation scenario.
    """
    obj_to_weight = {
        "uniform": None,
        "user_reward": [1.0, 0.0],
...
class TRIPConfigForNegotiation(TRIPConfig):
    combined_action = True
    special_tokens_dict = neg_special_tokens_dict
    n_topics = 5
    temperature = 0.000001
    # prompt for user-aware strategic planning for negotiation
    prompt = """
    ...
    """
    """
    class TRIP configuration for negotiation scenario.
    """
...
class TRIPConfigForEmotionalSupport(TRIPConfig):
    combined_action = False
    special_tokens_dict = es_special_tokens_dict
    temperature = 0.000001
    # prompt for user-aware strategic planning for negotiation
    prompt = """
    You are an expert in emotional support.
    Now give you a conversation history between a therapist and a patient, you need to infer the mental states
    and future actions of the patient.
    """
    cot_prompt = """
    The following is the conversation history: {}
    Question: What are the mental states and future actions of the patient.? Answer:
    """
    # chain of thought prompting
    rewrite_prompt = """
    Assume you are the expert analyst. Given the conversation history and a naive action instruction, 
...
class TRIPConfigForPersuation(TRIPConfig):
    combined_action = False
