...
from baselines.COLOR.modeling_bart import shift_tokens_right
from baselines.COLOR.modeling_bart import BartEncoder, BartDecoder
from baselines.COLOR.generation_utils import model_decode
from baselines.COLOR.utils import get_tokenizer
...
class COLORModel(Model):
    ...
    def __init__(self, model_config, **kwargs):
        """
        constructor for class COLOR model
        ...
        self.tokenizer, num_added_tokens, token_id_dict = get_tokenizer(self.model_config.tokenizer, name="bart")
        vocab_size = len(self.tokenizer)

        self.pad_token_id = token_id_dict["pad_token_id"]
        self.bos_token_id = token_id_dict["bos_token_id"]
        self.eos_token_id = token_id_dict["eos_token_id"]
        self.sep_token_id = token_id_dict["sep_token_id"]

        # create the config instance
        config = BartConfig.from_pretrained(self.model_config.plm,
                                            cache_dir=self.model_config.cached_dir)

        # create the plm backbone for COLOR model
        self.plm = COLOR(config=config, args=self.model_config)
        self.plm.resize_token_embeddings(vocab_size)

    def forward(self, batch):
        """
        forward function of the COLOR model
        :param batch: a batch of input data
        :return:
        """
        return self.plm(**batch)
...
class COLOR(BartPretrainedModel):
    ...
    def _logit(self, z_0, z_t, z_T, t_0, t_t, t_T, Z_u, delta_u):
        ...
        alpha = (t / (T + self.eps)).view(-1,
                                          1)  # (bsz, 1). The ratio between the middle time length and the whole time length
    ...
    def get_constrastive_loss(self, z_0, z_t, z_T, t_0, t, T, Z_u, delta_u):
        ...
        for pos_idx in range(z_T.shape[0]):
            count_pos += 1
            neg_i_logit = self._logit(z_0=z_0, z_t=z_t[pos_idx], z_T=z_T, t_0=t_0, t_t=t, t_T=T, Z_u=Z_u,
                                      delta_u=delta_u)
    ...
    def generate(self, inputs, tokenizer, args):
        ...
        if args.infer_use_bridge:
            if sample_number == 0:
                ...
                sampled_bridge_points = self.simulate_brownian_bridge(B_0=start_latents, B_T=end_latents,
                                                                      T=sample_number + 1, Z_u=Z_u, delta_u=delta_u)[1:]
