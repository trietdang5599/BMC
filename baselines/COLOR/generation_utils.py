...
def greedy_decoding(
    ...
    if dataset == "TGConv":
        ...
        if sample_number == 0:
            target_subgoal = inputs["target_subgoal"][0][0, 1:-1].tolist()
            return_list = tokenizer.convert_tokens_to_ids(
                tokenizer.tokenize("[A] Say goodbye [T] NULL")) + target_subgoal
            return_tensor = torch.tensor(return_list).unsqueeze(0).contiguous().to(device)
            return return_tensor
    ...
    if dataset == "TGConv":
        dec_ids = torch.tensor([tokenizer.bos_token_id, tokenizer.convert_tokens_to_ids("[T]")]).unsqueeze(0).repeat(
            batch_size, 1).contiguous().to(device)
    else:
        dec_ids = torch.tensor([tokenizer.bos_token_id, tokenizer.convert_tokens_to_ids("[A]")]).unsqueeze(0).repeat(
            batch_size, 1).contiguous().to(device)
    if bridge_points is not None and bridge_points_mask is not None:
        bridge_embeds = torch.full((bridge_points_mask.shape[0], bridge_points_mask.shape[1], model.latent_dim), 0,
                                   dtype=torch.float).to(input_ids.device)
