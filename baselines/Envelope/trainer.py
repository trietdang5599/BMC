...
from baselines.Envelope.data_processor import EnvelopeDataProcessorForRecommendation, EnvelopeTorchDataset, \
    EnvelopeDataProcessorForNegotiation, EnvelopeDataProcessorForEmotionalSupport
...
class EnvelopeTrainer(Trainer):
    ...
    def construct_dataloaders(self, data_instances, batch_size, goal2id, shuffle=True, num_workers=1):
        ...
        if self.game_config.name == RECOMMENDATION:
            convert_example_to_feature = EnvelopeDataProcessorForRecommendation
        # the data processor for the negotiation scenario
        elif self.game_config.name == NEGOTIATION:
            convert_example_to_feature = EnvelopeDataProcessorForNegotiation
        # the data processor for the emotional support conversation
        elif self.game_config.name == EMOTIONAL_SUPPORT:
            convert_example_to_feature = EnvelopeDataProcessorForEmotionalSupport
        else:
            raise Exception("Invalid scenario....")
        # construct the torch dataset
        torch_dataset = EnvelopeTorchDataset(
        ...
        for i in tqdm(range(self.model_config.num_train_ppo_epochs)):
            ...
            for batch in train_loader:
                ...
                with torch.no_grad():
                    ...
                    # w_{prev}  = w_{current}
                    # for envelope, there is no past learned preferences.
                    ...
                    # compute the objective embedding for previous sampled preferences
                ...
                # NOTE: PI TD error
                # actor_loss += (1 - self.model_config.alpha) * F.mse_loss(Q1.view(-1), Q_next_target.view(-1), reduction = 'mean')
        ...
        for train_step in range(0, self.model_config.num_train_rl_epochs + 1):
            ...
            if train_step >= 0:
                ...
                if dev_cases is not None and (train_step + 1) % 10 == 0:
                    ...
                    # # logging the results
                    # for logger in self.loggers:
                    #     logger.record(_, self.ppo_global_step)
