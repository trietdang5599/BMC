...
from baselines.TCP.ModelOutput import ModelOutput
...
class KTMutualAttention(nn.Module):
    ...
    def forward(
        ...
        # print(target_attn_weights.shape)
        # print(target_key_states.shape)
        # print(target_mask.shape)
        # print(bsz, 1, src_len, target_len)
        # assert 1==0
...
# modifying the information fusion layer due to lacking user profile
class InformationFusion(nn.Module):
    ...
    def forward(self, k_states, h_states, u_states):
        # ignoring the user profile hidden states
        gate1 = self.sigmod(self.gate_layer1(torch.cat([k_states, h_states], -1)))
        f_states = gate1 * k_states + (1 - gate1) * h_states
        # gate2 = self.sigmod(self.gate_layer2(torch.cat([u_states, states], -1)))
        # f_states = gate2 * u_states + (1 - gate2) * states
        # since the user profile and knowledge are not available during inference time
        # we ignore knowledge and user profile file of the TCP model.
        return h_states
...
class DecoderLayer(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.ht_mutual_attn = KTMutualAttention(
            embed_dim=self.embed_dim,
            num_heads=args.decoder_attention_heads,
            dropout=args.attention_dropout,
            init_std=args.init_std,
        ...
        self.ht_mutual_attn_layer_norm = nn.LayerNorm(self.embed_dim)
    ...
    def forward(
        ...
        # dot not consider the attention block for knowledge
        ...
        if hs_encoder_hidden_states is not None:
            ...
            # convext- target mutual attention
            # hs_hidden_states, _, hs_cross_attn_present_key_value = self.ht_mutual_attn(
        ...
        fused_hidden_states = self.fusion_layer(kg_hidden_states, hs_hidden_states, None)
...
class Planner(nn.Module):
    ...
    def forward(
        ...
        if kg_encoder_hidden_states is not None and kg_encoder_attention_mask is not None:
            # [bsz, seq_len] -> [bsz, 1, tgt_seq_len, src_seq_len]
            kg_encoder_attention_mask = _expand_mask(kg_encoder_attention_mask, inputs_embeds.dtype,
                                                     tgt_len=input_shape[-1])
        # expand encoder attention mask
        if up_encoder_hidden_states is not None and up_encoder_attention_mask is not None:
            # [bsz, seq_len] -> [bsz, 1, tgt_seq_len, src_seq_len]
            up_encoder_attention_mask = _expand_mask(up_encoder_attention_mask, inputs_embeds.dtype,
                                                     tgt_len=input_shape[-1])
        # expand encoder attention mask
        if hs_encoder_hidden_states is not None and hs_encoder_attention_mask is not None:
            # [bsz, seq_len] -> [bsz, 1, tgt_seq_len, src_seq_len]
            hs_encoder_attention_mask = _expand_mask(hs_encoder_attention_mask, inputs_embeds.dtype,
                                                     tgt_len=input_shape[-1])
