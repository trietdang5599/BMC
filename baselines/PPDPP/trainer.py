...
from baselines.PPDPP.data_processor import PPDPPDataProcessorForRecommendation, PPDPPTorchDatasetForRecommendation, \
    PPDPPTorchDatasetForNegotiation, PPDPPDataProcessorForNegotiation, PPDPPDataProcessorForEmotionalSupport, \
    PPDPPTorchDatasetForEmotionalSupport, PPDPPDataProcessorForPersuation, PPDPPTorchDatasetForPersuation
...
from config.constants import RECOMMENDATION, NEGOTIATION, EMOTIONAL_SUPPORT, SL_RATIO, SUCCESS_RATE, AVG_TURN, FAIRNESS, \
    TOXICITY, ITEM_FREQ, USER_REWARD, NEGOTIATION_GOAL2DESCRIPTION, ES_CONV_GOAL2DESCRIPTION, PERSUATION, DURECDIAL_GOAL2DESCRIPTION, P4G_GOAL2DESCRIPTION

from text_gen.wrapper_generation import WrapperGeneration
...
class PPDPPTrainer(Trainer):
    ...
    def construct_dataloaders(self, data_instances, batch_size, goal2id, shuffle=True, num_workers=1):
        ...
        if self.game_config.name == RECOMMENDATION:
            torch_dataset = PPDPPTorchDatasetForRecommendation(
                tokenizer=self.tokenizer,
                instances=data_instances,
                goal2id=goal2id,
                max_sequence_length=self.model_config.max_sequence_length,
                device=self.device,
                is_so_game  = self.game_config.is_so_game,
                convert_example_to_feature=PPDPPDataProcessorForRecommendation()
            )
        # negotiation scenario
        elif self.game_config.name == NEGOTIATION:
            torch_dataset = PPDPPTorchDatasetForNegotiation(
                tokenizer=self.tokenizer,
                instances=data_instances,
                goal2id=goal2id,
                max_sequence_length=self.model_config.max_sequence_length,
                device=self.device,
                is_so_game  = self.game_config.is_so_game,
                convert_example_to_feature=PPDPPDataProcessorForNegotiation()
            )
        # emotional support conversation
        elif self.game_config.name == EMOTIONAL_SUPPORT:
            torch_dataset = PPDPPTorchDatasetForEmotionalSupport(
                tokenizer=self.tokenizer,
                instances=data_instances,
                goal2id=goal2id,
                max_sequence_length=self.model_config.max_sequence_length,
                device=self.device,
                is_so_game  = self.game_config.is_so_game,
                convert_example_to_feature=PPDPPDataProcessorForEmotionalSupport()
            )
        elif self.game_config.name == PERSUATION:
             torch_dataset = PPDPPTorchDatasetForPersuation(
                tokenizer=self.tokenizer,
                instances=data_instances,
                goal2id=goal2id,
                max_sequence_length=self.model_config.max_sequence_length,
                device=self.device,
                is_so_game  = self.game_config.is_so_game,
                convert_example_to_feature=PPDPPDataProcessorForPersuation()
            )   
        else:
